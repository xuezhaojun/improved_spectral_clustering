...
# the authoer of this file is Chen Yijing
...
class GMeans(object):
    ...
    def _recursiveClustering(self, data, depth, index):
        """
        recursively run kmeans with k=2 on the data until a max_depth is reached or we have
            gaussian clusters
        """
        depth += 1
...
if __name__ == '__main__':
    ...
    start_time = time()
    gmeans = GMeans(min_obs=3000, random_state=1010, max_depth=10,
                    strictness=0)
    gmeans.fit(mnist_train)
    print("=" * 88)
    print("K = {} Time = {}".format(len(set(list(gmeans.labels_))), int(time() - start_time)))
    print("=" * 88)